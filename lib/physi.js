(function (root, factory) {
	if (typeof define === 'function' && define.amd) {
		...
		// like Node.
		module.exports = factory(require('three'),require('./physijs_worker'));
	} else {
		// Browser globals (root is window)
		root.Physijs = factory(root.THREE, root.Worker);
	}
}(this, function (THREE, Worker)

{
	...
	Physijs.Scene = function( params ) {
		...
		this._worker = new Worker( onmessageMain/*Physijs.scripts.worker || 'physijs_worker.js'*/ );
		this._worker.transferableMessageMain = function(a, b ){   //overrides

			 var e={data:a};
		 	 this.onmessageWorker(e) };//this._worker.webkitPostMessage || this._worker.postMessage;
		...
		this._worker.transferableMessageMain( ab, [ab] );
		SUPPORT_TRANSFERABLE = ( ab.byteLength === 0 );

		/*this._worker.onmessage =*/ function onmessageMain( event ) {
			var _temp,
				data = event.data;
		...
		params = params || {};
		params.ammo = Physijs.scripts.ammo || 'ammo_old.js';
	...
	Physijs.Scene.prototype._updateScene = function( data ) {
		...
		if ( SUPPORT_TRANSFERABLE ) {
			...
			this._worker.transferableMessageMain( data.buffer, [data.buffer] );
		}
	...
	Physijs.Scene.prototype.execute = function( cmd, params ) {
		this._worker.transferableMessageMain({ cmd: cmd, params: params });   // postMessage;
	};
	...
	// Phsijs.SkinnedMesh
	Physijs.SkinnedMesh = function ( geometry, material, mass ) {
		var index;
	...
	Physijs.SkinnedMesh.prototype = Object.create( THREE.SkinnedMesh.prototype)//new THREE.SkinnedMesh;
	Physijs.SkinnedMesh.prototype.constructor = Physijs.SkinnedMesh;
	Eventable.make( Physijs.SkinnedMesh );

	// Physijs.SkinnedMesh.mass
	Physijs.SkinnedMesh.prototype.__defineGetter__('mass', function() {
		return this._physijs.mass;
	});
	Physijs.SkinnedMesh.prototype.__defineSetter__('mass', function( mass ) {
		this._physijs.mass = mass;
		if ( this.world ) {
			this.world.execute( 'updateMass', { id: this._physijs.id, mass: mass } );
		}
	...
	// Physijs.SkinnedMesh.applyCentralImpulse
	Physijs.SkinnedMesh.prototype.applyCentralImpulse = function ( force ) {
		if ( this.world ) {
			this.world.execute( 'applyCentralImpulse', { id: this._physijs.id, x: force.x, y: force.y, z: force.z } );
		}
	};

	// Physijs.SkinnedMesh.applyImpulse
	Physijs.SkinnedMesh.prototype.applyImpulse = function ( force, offset ) {
		if ( this.world ) {
			this.world.execute( 'applyImpulse', { id: this._physijs.id, impulse_x: force.x, impulse_y: force.y, impulse_z: force.z, x: offset.x, y: offset.y, z: offset.z } );
		}
	};

	// Physijs.SkinnedMesh.applyTorque
	Physijs.SkinnedMesh.prototype.applyTorque = function ( force ) {
		if ( this.world ) {
			this.world.execute( 'applyTorque', { id: this._physijs.id, torque_x: force.x, torque_y: force.y, torque_z: force.z } );
		}
	};

	// Physijs.SkinnedMesh.applyCentralForce
	Physijs.SkinnedMesh.prototype.applyCentralForce = function ( force ) {
		if ( this.world ) {
			this.world.execute( 'applyCentralForce', { id: this._physijs.id, x: force.x, y: force.y, z: force.z } );
		}
	};

	// Physijs.SkinnedMesh.applyForce
	Physijs.SkinnedMesh.prototype.applyForce = function ( force, offset ) {
		if ( this.world ) {
			this.world.execute( 'applyForce', { id: this._physijs.id, force_x: force.x, force_y : force.y, force_z : force.z, x: offset.x, y: offset.y, z: offset.z } );
		}
	};
	//hack
	Physijs.SkinnedMesh.prototype.clearForces = function () {
		if ( this.world ) {
			this.world.execute( 'clearForces', { id: this._physijs.id} );
		}
	};

	//end hack
	//hack
	Physijs.SkinnedMesh.prototype.getTotalForce = function () {
		if ( this.world ) {
			var totalForce = {};
			this.world.execute( 'getTotalForce', { id: this._physijs.id, totalForce: totalForce} );
	...
	// Physijs.SkinnedMesh.getAngularVelocity
	Physijs.SkinnedMesh.prototype.getAngularVelocity = function () {
		//hack
		if ( this.world ) {
			this.world.execute( 'getAngularVelocity', { id: this._physijs.id, angularVelocity: this._physijs.angularVelocity} );//return { x:_vector.x(),y:_vector.y(),z: _vector.z()};
		}
	...
	// Physijs.SkinnedMesh.setAngularVelocity
	Physijs.SkinnedMesh.prototype.setAngularVelocity = function ( velocity ) {
		if ( this.world ) {
			this.world.execute( 'setAngularVelocity', { id: this._physijs.id, x: velocity.x, y: velocity.y, z: velocity.z } );
		}
	};

	// Physijs.SkinnedMesh.getLinearVelocity
	Physijs.SkinnedMesh.prototype.getLinearVelocity = function () {
		//hack
		///var vector = {}
		if ( this.world ) {
			this.world.execute( 'getLinearVelocity', { id: this._physijs.id, linearVelocity: this._physijs.linearVelocity} );//return { x:_vector.x(),y:_vector.y(),z: _vector.z()};
		}
        //end hack
		return this._physijs.linearVelocity;
	...
	// Physijs.SkinnedMesh.setLinearVelocity
	Physijs.SkinnedMesh.prototype.setLinearVelocity = function ( velocity ) {
		if ( this.world ) {
			this.world.execute( 'setLinearVelocity', { id: this._physijs.id, x: velocity.x, y: velocity.y, z: velocity.z } );
		}
	};

	// Physijs.SkinnedMesh.setAngularFactor
	Physijs.SkinnedMesh.prototype.setAngularFactor = function ( factor ) {
		if ( this.world ) {
			this.world.execute( 'setAngularFactor', { id: this._physijs.id, x: factor.x, y: factor.y, z: factor.z } );
		}
	};

	// Physijs.SkinnedMesh.setLinearFactor
	Physijs.SkinnedMesh.prototype.setLinearFactor = function ( factor ) {
		if ( this.world ) {
			this.world.execute( 'setLinearFactor', { id: this._physijs.id, x: factor.x, y: factor.y, z: factor.z } );
		}
	};

	// Physijs.SkinnedMesh.setDamping
	Physijs.SkinnedMesh.prototype.setDamping = function ( linear, angular ) {
		if ( this.world ) {
			this.world.execute( 'setDamping', { id: this._physijs.id, linear: linear, angular: angular } );
		}
	};

	// Physijs.SkinnedMesh.setCcdMotionThreshold
	Physijs.SkinnedMesh.prototype.setCcdMotionThreshold = function ( threshold ) {
		if ( this.world ) {
			this.world.execute( 'setCcdMotionThreshold', { id: this._physijs.id, threshold: threshold } );
		}
	};

	// Physijs.SkinnedMesh.setCcdSweptSphereRadius
	Physijs.SkinnedMesh.prototype.setCcdSweptSphereRadius = function ( radius ) {
		if ( this.world ) {
			this.world.execute( 'setCcdSweptSphereRadius', { id: this._physijs.id, radius: radius } );
		}
	};
	...
	// Physijs.SkinnedCapsuleMesh
	Physijs.SkinnedCapsuleMesh = function( geometry, material, mass ) {
		var width, height, depth;

		Physijs.SkinnedMesh.call( this, geometry, material, mass );
		...
		this._physijs.type = 'capsule';
		//hack
		this._physijs.radius = height/4//Math.max(width / 2, depth / 2);
		//end hack
		this._physijs.height = height;
		this._physijs.mass = (typeof mass === 'undefined') ? width * height * depth : mass;
	};
	Physijs.SkinnedCapsuleMesh.prototype = new Physijs.SkinnedMesh;
	Physijs.SkinnedCapsuleMesh.prototype.constructor = Physijs.SkinnedCapsuleMesh;


	// Physijs.SkinnedBoxMesh
	Physijs.SkinnedBoxMesh = function( geometry, material, mass ) {
		var width, height, depth;
	...
	Physijs.SkinnedBoxMesh.prototype = new Physijs.SkinnedMesh;
	Physijs.SkinnedBoxMesh.prototype.constructor = Physijs.SkinnedBoxMesh;
