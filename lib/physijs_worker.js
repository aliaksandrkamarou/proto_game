(function (root, factory) {
	if (typeof define === 'function' && define.amd) {
		...
		// like Node.
		module.exports = factory(require('three'),require('./ammo_old'));
	} else {
		// Browser globals (root is window)
		root.Worker = factory(root.THREE, root.Ammo);
	}
}(this, function (THREE, Ammo)
{
'use strict';

 function Worker(pstMsgWork ){

 	
var	transferableMessageWorker = function ( a , b ){
	var e = {data:a};

 	pstMsgWork(e) },//this.webkitPostMessage || this.postMessage,
...
transferableMessageWorker( ab, [ab] );
...
//	importScripts( params.ammo );
	...
	var collisionConfiguration = new Ammo.btDefaultCollisionConfiguration,
		...
		broadphase;
	  //  solver.setRandSeed(5)
	  //  console.log(solver.getRandSeed())
	...
	transferableMessageWorker({ cmd: 'worldReady' });
};
...
// end hack
	...
	transferableMessageWorker({ cmd: 'objectReady', params: body.id });
};
...
//hack
public_functions.clearForces = function (details){
	//console.log(_objects[details.id].getTotalForce().x());
	//_objects[details.id].clearForces();
	//_objects[details.id].deactivate();
	//console.log('clear forces !!!!!!!!!!!!!!!!!!!!!!!!!!!!')

}
// hack
public_functions.getTotalForce = function (details) {
	 _vector = _objects[details.id].getTotalForce();
	 details.totalForce.x = _vector.x();
	 details.totalForce.y = _vector.y();
	 details.totalForce.z = _vector.z();
...
// hack
	 public_functions.getAngularVelocity = function (details) {
		 _vector = _objects[details.id].getAngularVelocity();
		 details.angularVelocity.x = _vector.x();
		 details.angularVelocity.y = _vector.y();
		 details.angularVelocity.z = _vector.z();
...
// hack
	 public_functions.getLinearVelocity = function (details) {
		 _vector = _objects[details.id].getLinearVelocity();
		 details.linearVelocity.x = _vector.x();
		 details.linearVelocity.y = _vector.y();
		 details.linearVelocity.z = _vector.z();
...
public_functions.simulate = function simulate( params ) {
	if ( world ) {
		...
		if ( !params.timeStep ) {
			if ( last_simulation_time ) {
				...
				while ( params.timeStep + last_simulation_duration <= fixedTimeStep ) {
					params.timeStep = ( Date.now() - last_simulation_time ) / 1000; // time since last simulation // block loop!!!
				}
			} else {
		...
		//console.log('params.timeStep '+params.timeStep+ ' , params.maxSubSteps '+params.maxSubSteps+  ' , fixedTimeStep '+fixedTimeStep)
...
/*
			worldreport[ offset + 1 ] = round(origin.x(),6);
			worldreport[ offset + 2 ] = round(origin.y(),6);
			worldreport[ offset + 3 ] = round(origin.z(),6);
*/
...
/*
			worldreport[ offset + 4 ] = round(rotation.x(),6);
			worldreport[ offset + 5 ] = round(rotation.y(),6);
			worldreport[ offset + 6 ] = round(rotation.z(),6);
			worldreport[ offset + 7 ] = round(rotation.w(),6);
*/
//end hack
	...
	if ( SUPPORT_TRANSFERABLE ) {
		transferableMessageWorker( worldreport.buffer, [worldreport.buffer] );
	} else {
		transferableMessageWorker( worldreport );
	}
...
reportCollisions = function() {
	...
	for ( i = 0; i < num; i++ ) {
		...
		for ( j = 0; j < num_contacts; j++ ) {
			...
			//}
				...
				transferableMessageWorker( _objects_ammo );
	...
	if ( SUPPORT_TRANSFERABLE ) {
		transferableMessageWorker( collisionreport.buffer, [collisionreport.buffer] );
	} else {
		transferableMessageWorker( collisionreport );
	}
};
...
reportVehicles = function() {
	...
	if ( j !== 0 ) {
		if ( SUPPORT_TRANSFERABLE ) {
			transferableMessageWorker( vehiclereport.buffer, [vehiclereport.buffer] );
		} else {
			transferableMessageWorker( vehiclereport );
		}
	}
};
...
reportConstraints = function() {
	...
	if ( i !== 0 ) {
		if ( SUPPORT_TRANSFERABLE ) {
			transferableMessageWorker( constraintreport.buffer, [constraintreport.buffer] );
		} else {
			transferableMessageWorker( constraintreport );
		}
	}

};

this.onmessageWorker = function( event ) {
